...
function getCacheHitRate(usage: GLMUsage): string {
	const cacheHit = getCacheHitTokens(usage);
	const cacheMiss = getCacheMissTokens(usage);
...
function getCacheMissTokens(usage: GLMUsage): number {
	return (
		usage.prompt_cache_miss_tokens ?? Math.max(usage.prompt_tokens - getCacheHitTokens(usage), 0)
	);
}
...
export function createCacheTraceSnapshot(
	request: GLMRequest,
	inputMessages: readonly vscode.LanguageModelChatRequestMessage[] = [],
	requestKind: RequestKind = classifyGLMRequest({ request, inputMessages }),
): CacheTraceSnapshot {
...
export function getCacheTraceWarnings(snapshot: CacheTraceSnapshot): string[] {
	...
	if (snapshot.stats.missingToolReasoningMessages > 0) {
		warnings.push(
			`${snapshot.stats.missingToolReasoningMessages} assistant tool-call message(s) are missing marker-replayed reasoning_content; GLM requires this in thinking tool-call histories and cache prefixes may drift.`,
		);
	}
...
function summarizeMessages(messages: GLMMessage[]): CacheTraceMessageSummary[] {
...
function summarizeTools(tools: GLMTool[]): CacheTraceToolSummary[] {
...
function summarizeStats(messages: GLMMessage[], toolCount: number): CacheTraceStats {
	let userMessages = 0;
