...
export interface DumpGLMRequestOptions {
 ...
 *   glm-request-<timestamp>-NNNN.input.json     — VS Code input snapshot
 *   glm-request-<timestamp>-NNNN.resolved.json  — post-vision VS Code snapshot
	...
	return createDumpSnapshot({
		...
		vision:
			stage === 'resolved'
				...
				: undefined,
		deepSeekPromptSummary: summarizeGLMSystemPrompt(request.messages),
		messages,
		requestOptions: options.requestOptions,
	});
}
...
function summarizeGLMSystemPrompt(messages: readonly GLMMessage[]): SystemPromptSummary {
	const message = messages[0];
	const customizations = summarizeGLMCustomizations(messages);
	if (!message) {
		return createSystemPromptSummary(null, null, '', customizations);
	}
...
function summarizeGLMCustomizations(messages: readonly GLMMessage[]): CustomizationsSummary {
	let customizationsUpdateCountInHistory = 0;
	let latestUserMessageIndex: number | null = null;
	let latestUserHasCustomizationsUpdate = false;
...
function summarizeRequestOptions(options: vscode.ProvideLanguageModelChatResponseOptions): object {
	...
	const configuration = sanitizeJsonValue(providerOptions.configuration);
	return {
		...
		configuration,
		configurationKeys: getObjectKeys(configuration),
	};
}
...
function logRequestDump(
	...
	const systemPromptSummary = summarizeGLMSystemPrompt(request.messages);
	logger.debug(
		formatRequestLogLine(
			requestKind,
...
function getRequestDumpBaseRootUri(globalStorageUri: vscode.Uri): vscode.Uri {
	...
	return vscode.Uri.file(join(tmpdir(), 'glm-request-dumps'));
}
