...
const ACTION_URLS: readonly ActionUrlDefinition[] = [
	{
		...
		handle: () => vscode.commands.executeCommand('openrouter-for-copilot.setApiKey'),
		resolveFailureMessage: 'Failed to resolve OpenRouter set API key URI',
	},
	{
		key: 'showLogs',
		path: SHOW_LOGS_URI_PATH,
		handle: () => logger.show(),
		resolveFailureMessage: 'Failed to resolve OpenRouter show logs URI',
		setUrl: setProviderNoticeShowLogsUrl,
	},
	{
		path: SET_VISION_MODEL_URI_PATH,
		handle: () => vscode.commands.executeCommand('openrouter-for-copilot.setVisionModel'),
		resolveFailureMessage: 'Failed to resolve OpenRouter set vision model URI',
			...
			handleUri(uri) {
				...
				if (action) {
					void Promise.resolve(action.handle()).catch((error) => {
						logger.warn(`Failed to handle OpenRouter URI action: ${uri.path}`, error);
					});
					return;
				}
				logger.warn(`Unhandled OpenRouter URI: ${uri.toString(true)}`);
			},
		}),
	);
